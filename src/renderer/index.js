document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 3. Placeholder pour l'Interactivité de la Liste ---
    document.querySelectorAll('.clip-item').forEach(item => {
        item.addEventListener('click', function() {
            // Logique de sélection dans l'historique
            document.querySelectorAll('.clip-item').forEach(i => i.classList.remove('active'));
            this.classList.add('active');
            
            // NOTE: Ici irait la logique Electron/JS pour charger les données réelles du clip
            // et mettre à jour la vue 'details-content' avec le texte complet.
            console.log('Clip sélectionné (ID mock):', this.getAttribute('data-id'));
        });
    });
